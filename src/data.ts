// Shared data & tokens for OpenSwitch.
// Authorizations are stored as short opaque aliases; nothing is ever sent anywhere.
...
export const OS_TOOLS_BY_ID: Record<ToolId, Tool> = Object.fromEntries(
  OS_TOOLS.map((t) => [t.id, t]),
...
export const OS_POOL: Credential[] = [
  // Git
...
export type EnvBindings = Partial<Record<ToolId, string>>;
...
export const OS_ENVS: Environment[] = [
  {
    id: 'personal',
    name: 'Personal',
...
export const OS_ENVS_BY_ID: Record<string, Environment> = Object.fromEntries(
  OS_ENVS.map((e) => [e.id, e]),
);

export const OS_POOL_BY_ID: Record<string, Credential> = Object.fromEntries(
  OS_POOL.map((c) => [c.id, c]),
);

export function osCredsForTool(toolId: ToolId): Credential[] {
  return OS_POOL.filter((c) => c.tool === toolId);
}

export function osFmtDate(iso: string): string {
